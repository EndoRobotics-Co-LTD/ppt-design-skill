...
1. **목적/유형 확인** — 경영 보고, 제품 세일즈, 주간 보고, 교육 등 어떤 PPT인지
2. **자료/주제 수집** — 핵심 메시지, 슬라이드 개요, 첨부 자료 경로
3. **개요 슬라이드 제안** — AI가 슬라이드 타이틀 리스트를 먼저 제시 → 사용자 승인
4. **순차 생성** — 한 번에 1~3장씩 만들고 사용자가 확인할 시간을 둔다
5. **저장 시점** — 사용자가 "저장해줘"라고 명시할 때까지 절대 자동 저장하지 않음
...
template = Path("Reference/EndoRobotics_PPT Design 01_2026.02.pptx")
with LiveSession.open(template) as session:
    # 표지
    session.add_cover(
...
```
PPTMaker/
├── Reference/                              ← 사내 표준 .pptx 템플릿 (2026.02 버전)
├── design_tokens.json                      ← 추출된 디자인 토큰 (SSOT)
├── src/pptmaker/
...
│       └── ... (12 본문 레이아웃)
├── scripts/
│   ├── analyze_templates.py                ← Reference 분석 → design_tokens.json
...
- Reference 템플릿 갱신 시: 새 .pptx를 `Reference/`에 넣고 `python scripts/analyze_templates.py` 실행 → `design_tokens.json` 자동 갱신
