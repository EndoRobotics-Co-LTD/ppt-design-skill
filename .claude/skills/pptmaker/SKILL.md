...
2. **테마 선택** (시나리오 A에서만) — 어느 테마를 쓸지 확인:
...
3. **목적/유형 확인** — 경영 보고, 제품 세일즈, 주간 보고, 교육 등 어떤 PPT인지
4. **자료/주제 수집** — 핵심 메시지, 슬라이드 개요, 첨부 자료 경로
5. **개요 슬라이드 제안** — AI가 슬라이드 타이틀 리스트를 먼저 제시 → 사용자 승인
6. **순차 생성** — 한 번에 1~3장씩 만들고 사용자가 확인할 시간을 둔다. PowerPoint 화면에서 실시간으로 슬라이드가 추가되는 것을 사용자가 직접 본다.
7. **저장 시점** — 사용자가 "저장해줘"라고 명시할 때까지 절대 자동 저장하지 않음. 단, 시나리오 A는 시작 시점에 이미 작업 경로의 사본이 만들어진 상태이므로, 사용자가 PowerPoint에서 직접 Ctrl+S를 눌러도 안전 (템플릿은 안 건드림).
