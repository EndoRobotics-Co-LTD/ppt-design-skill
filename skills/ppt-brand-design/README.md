...
- 기본 동작: **현재 폴더의 `.claude/skills/ppt-brand-design/` 에 설치** (project-local).
- 글로벌 설치는 `-g` 추가: `npx skills add EndoRobotics-Co-LTD/endo-skills -s ppt-brand-design -g`
...
- 자동으로 글로벌(`~/.claude/skills/ppt-brand-design/`)에 설치 + `pip install -e .` + 검증까지.
- 한 방에 끝.
...
| **Git** | 설치되어 있어야 함 |
| **Claude Code** | 최신 버전 |
| **GitHub 접근권** | `EndoRobotics-Co-LTD/endo-skills` 리포 (public, 인증 불필요) |
...
- 기본 동작: 현재 폴더의 `.claude/skills/ppt-brand-design/` 에 설치 (project-local).
- 글로벌은 `-g` 추가.
- ⚠️ Python 의존성은 별도 설치 필요: `cd <설치위치>; python -m pip install -e .`

### 방법 B — PowerShell 부트스트랩 (Python 의존성도 자동)
...
2. `endo-skills` repo 클론 후 `skills/ppt-brand-design/` 만 추출 → `~/.claude/skills/ppt-brand-design/`
3. Python 패키지 설치 (`python-pptx`, `pywin32`, `pptmaker`)
4. 설치 검증

마지막에 `Installation complete: ppt-brand-design` 메시지가 보이면 OK.
...
# 2) ppt-brand-design 만 정확한 위치로 이동
Move-Item "$temp\skills\ppt-brand-design" "$env:USERPROFILE\.claude\skills\ppt-brand-design"
...
```
~/.claude/skills/ppt-brand-design/user_layouts/    ← gitignored, 사용자별 fork
...
새 디자인 가이드 / 새 레이아웃 / 버그 수정이 배포되면 **재설치** 하면 됩니다:

```powershell
# 1) 기존 설치 제거
Remove-Item -Recurse -Force $env:USERPROFILE\.claude\skills\ppt-brand-design

# 2) 다시 설치 (위 한 줄 인스톨러)
irm https://raw.githubusercontent.com/EndoRobotics-Co-LTD/endo-skills/main/skills/ppt-brand-design/install.ps1 | iex
```

Claude Code 재시작 → 새 버전 적용.

> 💡 직접 `git pull`은 작동하지 않음 (이 폴더는 endo-skills repo의 일부만 추출된 형태). `user_layouts/`(사용자 커스텀)는 재설치 전에 백업 권장.
...
| 증상 | 해결 |
|---|---|
| `/ppt-brand-design` 자동완성이 안 보임 | Claude Code 완전 종료 후 재시작. 그래도 안 되면 `~/.claude/skills/ppt-brand-design/SKILL.md` 파일 존재 확인. 디렉토리 이름이 다르면 install.ps1 또는 `npx skills add` 로 재설치 |
| Python 임포트 에러 (`ModuleNotFoundError: pptmaker`) | `cd $env:USERPROFILE\.claude\skills\ppt-brand-design; python -m pip install -e .` 재실행 |
