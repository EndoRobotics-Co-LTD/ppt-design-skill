...
1. **⚠️ 스킬 적용 여부 게이트** (반드시 다른 어떤 질문보다 먼저).
   - 질문: "**EndoRobotics PPT 스킬(사내 디자인 표준)을 적용할까요?**" — 짧게 Y/N로 물어봐도 됨.
   - **Yes (적용)** → 이 스킬을 활성화. 아래 2~8 단계 진행. **사내 디자인 표준(theme1/theme2 chrome, 14종 레이아웃, 맑은 고딕, 강제 색상)을 100% 강제**한다. 사용자가 폰트·색상·레이아웃을 깨려 해도 거부하고 표준 안내. (사외 발표·IR·내부 공식 보고 등 공식 자료에 적합)
   - **No (적용 안 함)** → **이 스킬은 사용하지 않는다.** 사용자에게 명확히 안내: "EndoRobotics 디자인 가이드라인 우회. 일반 PPT 작성으로 진행합니다." 그 후 일반 PPT 작성 모드로 전환 (Claude 자체 판단 또는 `anthropic-skills:pptx`). (개인 작업·임시 자료·연습용·외부 템플릿 사용 등에 적합)
   - **목적**: 사용자가 매 PPT 작성 시작 시 명시적으로 "사내 표준 강제 모드 / 자유 모드"를 선택. 비공식 작업에 표준이 강제되어 답답해지는 것 방지 + 공식 자료의 디자인 일관성은 무조건 보장.
...
2. **작업 시나리오 확인** — 새 PPT인지, 이어작업인지:
   - **A. 새 PPT 시작** → `LiveSession.new(theme=..., working_path=...)`
     ...
     - ⚠️ 같은 `working_path`가 이미 존재하거나 PowerPoint에 열려있으면 `FileExistsError`/`RuntimeError` — 절대 silent overwrite 안 함.
   - **B. 기존 작업 이어가기** → `LiveSession.open_existing(path)`
     - 사용자가 작업 중인 .pptx 경로를 받아 그대로 연다.
     - 기존 슬라이드 보존, 추가 작업은 끝에 append.
   - **C. ⚠️ 후속 수정·추가 요청** (가장 빈번한 시나리오):
     - 사용자가 이미 만든 PPT에 대해 "이 부분 바꿔줘", "한 장 더 추가해줘", "이 슬라이드 지워줘", "순서 바꿔줘" 같은 요청을 하면 → **반드시 `open_existing()` 사용**. 절대 `new()` 호출 금지.
     - 이유: `new()`는 처음부터 재생성이라 사용자가 PowerPoint에서 수동 편집한 내용 + 이미 만들어진 슬라이드가 모두 사라짐.
     - "처음부터 다시 만들어줘" 같이 사용자가 **명시적으로 재빌드를 요청**할 때만 `new(force_overwrite=True)`.
...
### 1.3 콘텐츠 SSOT (Single Source of Truth)

**콘텐츠의 정답지는 `.pptx` 파일 자체.** `.py` 스크립트는 절대 "전체 콘텐츠 정의"를 들고 있으면 안 된다.

원칙:
- 사용자가 PPT를 수정하면 `.pptx`가 최신 상태 (PowerPoint에서 직접 편집한 내용 포함).
- 매 수정 요청 시 Claude는 **임시 작업 스크립트를 만들어 실행하고, 끝나면 정리**한다. 작업 폴더에 `.py` 누적 금지.
- `.py`로 콘텐츠 정의를 보관하면 `.pptx`와 sync 깨지면서 stale 위험 — 절대 하지 않음.
- 수정 이력 관리는 `.pptx` 파일 자체로 (PowerPoint 변경 추적, 또는 `.pptx`를 git/Dropbox에 두기).

### 1.4 시각 건강 검진 (디자인 vs 컨텐츠 충돌 처리)

⚠️ **핵심 원칙**: 디자인 표준 강제 vs 실데이터 fit 이 충돌하면 **컨텐츠 완결성·가독성 우선**. 즉 Type 스케일을 일시적으로 한 단계 깨더라도 텍스트가 보이게 한다.

**자동 처리 (1단계)** — 모든 `add_xxx()` 메서드는 호출 끝에 자동으로 `_post_add_hook()` → `auto_fit()` 호출. 다음을 silent 처리:
- 텍스트가 박스 하단 잘림 → 박스 높이 확장 (`BODY_BOTTOM` 한도 내) → 폰트 한 단계 축소
- 본문 박스가 풋바(`BODY_BOTTOM`) 침범 → 박스 높이 줄임
- 적용된 fix는 콘솔에 로그 (silent change 방지)

**인터랙티브 fallback (3단계)** — 자동 fit 으로 해결 안 되는 issue는 `session._last_remaining_issues` 에 누적.

**Claude는 매 슬라이드 추가 후 반드시 다음을 확인**:
```python
session.add_xxx(...)
if session._last_remaining_issues:
    # 사용자에게 옵션 제시
    # A. 텍스트 축약 또는 요약
    # B. 다음 슬라이드로 분할
    # C. 디자인 룰 한 단계 더 완화 (CAPTION 사이즈로 더 축소 등)
    # D. 그대로 두기 (사용자 책임)
```

남는 issue 가 있으면 그대로 다음 작업으로 넘어가지 말고 사용자에게 안내한 뒤 결정을 받는다. 자동 처리든 사용자 결정이든 명확하게 로그/콘솔에 알린다 (silent 변경 금지).

### 1.5 부분 수정 — 후속 요청 처리

이미 만든 PPT의 부분만 수정할 때 사용할 메서드:

```python
# 기존 PPT 열기 (필수 — 절대 new() 호출 금지)
with LiveSession.open_existing(r"~/Desktop/2026Q1.pptx") as s:
    # 텍스트 부분 치환
    s.replace_text_in_slide(5, "+18%", "+22%")        # 슬라이드 5의 +18%를 +22%로

    # 전역 치환 (전체 슬라이드 대상)
    s.replace_text_global("2026 Q1", "2026 Q2")

    # 슬라이드 삭제
    s.delete_slide(7)

    # 슬라이드 순서 변경
    s.move_slide(from_index=5, to_index=3)

    # 슬라이드 본문 미리 확인 (어떤 텍스트가 있는지)
    text = s.get_slide_text(5)

    # 새 슬라이드 추가 (closing 직전에 append)
    s.add_kpi_cards("Q2 핵심 지표", [...])

    # 저장
    s.save()
```

**중요**: 사용자가 PowerPoint에서 슬라이드를 수동 편집했어도 모두 보존됨. Claude는 수정 명령만 적용.
